...
Run a `dev/build?flush`, and go into the CMS. Change the `Regions` page to type `RegionsPage` (on the Settings tab for that page). Now go back to your website and click on "Regions." You should see the new layout.

Again all of this is static content, and we need to start carving it up into something editable in the CMS. One of the most apparent features of this content is that it clearly belongs in a loop. We see the regions _Northeast_, _Southeast_, etc., all with the same structure and data. Each contains a title, a photo, and a short description.

We could hand this off to the rich text editor and create all this content in the `$Content` block, but that would be pretty unpleasant to deal with. Our content editor would have to take care to create very specific markup, and that would push the boundaries of WYSIWYG utility. We need to make each of these regions editable pieces of structured data.
...
```
As described in a previous tutorial, the `$owns` array is used to ensure that when regions are saved, their related images are published as well. Files and images are, by default, draft until explicitly published. This ensures they'll be implicitly published with the page that uses them.

You might have noticed that our `getCMSFields()` function looks a bit different. That's because we're not going to be using the typical page editing interface for this object, so we're not going to have the tabs that come with Page objects. We could very easily create one, but since this data type is so simple, we'll just create a simple field list and add all of our desired form fields as parameters to its constructor.
...
`$has_many` relationships are a bit of a special case because they have to be _reciprocated_ by the related class. While every `RegionsPage` **has many** _Regions_, it is also true that every `Region` **has one** _RegionPage_ that contains it. Regions cannot belong to any more than one region page.
...
You'll probably notice that we did not include the `$owns` variable in this update. This is because we do not want to publish any changes to a `RegionsPage` each time we save a `Region`. There could be a work in progress with the content on that page which isn't ready to be published yet, for example.
...
Let's take a look at the parameters passed to `GridField`'s constructor:

*   **'Regions'**: A required, arbitrary name for the GridField. You'll also need this if you ever want to make updates to your GridField after it's been added to the FieldList.
...
*   **GridFieldConfig_RecordEditor::create()**: This is a bit more complex. It creates an object that contains a number of `GridFieldComponent` objects, which provide various UI and functionality tools to the grid, such as pagination, an "add new" button, delete/edit buttons, etc. These `GridFieldConfig` objects can be configured with any variety of components you like, but SilverStripe ships with a few common configurations that are often used. `GridFieldConfig_RecordEditor` is a great one, because it provides all the basic UI you'd expect to have for managing data in a relationship such as this.

### Adding versioning

Right now, any changes we make to regions will go to the live site immediately, since they have no published/draft state. This might not be desirable! Let's start by adding versioning to those records.
...
Lastly, we need to declare ownership between `RegionsPage` and `Region` so that publication cascades from parent to child.
...
Notice that we don't need to use the fully-qualified class name supplied by `Region::class` in the `$owns` declaration. That's because we're referring to the relationship, not the class. The relationship is declared as `Regions` in the `has_many` array. The entries in `$owns` could just as easily be method names, if you wanted to declare ownership in a custom getter (returning a DataList).
...
Because we've changed a `private static` variable, we need to run a _?flush_ to see the update to the grid. How does it look? Pretty bad, right? The photo is not being resized to fit the grid.
...
We could have saved ourselves a lot of time by using a method that SilverStripe provides to all `Image` objects by default -- `CMSThumbnail()`. `'Photo.CMSThumbnail'` would have achieved a very similar result, but without the fallback text _(no image)_.
...
_themes/one-ring/templates/Layout/RegionsPage.ss, line 9+_
