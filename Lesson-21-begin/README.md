...
It is also possible to store the `.env` file in the SilverStripe installation root, in the case that you do not have the needed permission, or your site needs to be contained to a subdirectory.
</div>
...
To log errors, just access a concrete logging class through `Injector`, via the logger interface.
...
It's a good idea to throw these in your user code where appropriate, as logging is very useful for debugging. When using the logger in your classes, you need not keep going through `Injector` to get a logger instance. A cleaner approach is to inject the logger as a _dependency_.
